...
\plnote{Is it OK to include the unit type in this section?}
...
Any pair of pre-types $\jd{ \Gamma |- A, B \pretype}$ has a coproduct pretype $A+B$, satisfying the usual rules for elimination into pre-types, including the judgemental uniqueness rule: whenever $\jd{ \Gamma, w : A + B |- c(w) : C(w)}$, we have $c(w) \jdeq \copair{x . c( \injone x) }{y . c ( \injtwo y)}(w)$.  \plnote{Maybe it’s more natural to take the empty type, and then recover coprods as pushouts over that?  “Pushouts and an initial object” is a classic combo.}
...
\begin{mathpar}
  \inferrule{\ }{\Gamma \vdash \ivl \pretype}
\and
  \inferrule{\ }{\Gamma \vdash 0 : \ivl}
\and
  \inferrule{\ }{\Gamma \vdash 1 : \ivl}
\and
 \inferrule{\ }{\Gamma \vdash \cof{\copair{0}{1}}{\bool}{\ivl}}
\end{mathpar}
...
\todo{Section out of date.  New version should contain just the definition of Id as an instence of Extn, and then the assumption of J for it.}
