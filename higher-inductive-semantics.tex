...
\todo{Hmm\dots I guess we need non-fibrant HITs if we want to say it this way, which makes for an annoying sort of almost-circularity in exposition.  If we want to describe \emph{general} non-fibrant HITs that include this pushout, described in an analogous way to the fibrant ones, then I guess we ought to allow \emph{any} maps in place of cofibrations, so that by using $2\to \unittype$ we could ``glue in an $\fEq$''.  But that seems kind of like cracking nuts with a sledgehammer; maybe we ought to just give the rules for the non-fibrant pushout explicitly, since it is all we need.}
...
\msnote{Curiously, it seems to me as though the conservativity result for HTS using presheaves ought to also model non-fibrant \emph{colimits}, since the presheaf category is the free cocompletion.  I'm not sure whether that means anything.}
