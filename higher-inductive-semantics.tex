...
If $A\in \P(\Gamma)$, we write $\Gamma.A$ for the domain of $\xi(A)$.
Finally, for a morphism $\sigma : \Delta \to \Gamma$ in \E and $A\in\P(\Gamma)$ or $\T(\Gamma)$, we write $\sigma_A : A[\sigma] \to A$ for a cartesian morphism lying over $\sigma$.
...
Large swaths of the definitions below are taken almost verbatim from~\cite{lw:localuniv}.

\begin{definition}
  Let \E be a HCC.
  \begin{itemize}
  \item Given a pretype $A$ in context $\Gamma$ and a pretype $B$ in context $\Gamma.A$, a \textbf{dependent product} is a pretype $\prod[A,B]$ in context $\Gamma$ and a map $\fapp_{A,B} : \prod[A,B][\chi(A)] \to B$ in context $\Gamma.A$ such that for each section $t:\Gamma.A \to \Gamma.A.B$ there is a chosen section $\lambda(t):\Gamma \to \Gamma.\prod[A,B]$ such that $(\Gamma.A.\fapp_{A,B})\circ (1_{\Gamma.A},\lambda(t)) = t$.\msnote{Should we include $\eta$ here?  And the same question below.}
  \item \E has \textbf{weakly stable dependent products} if any $A$ and $B$ have a dependent product such that for any $\sigma:\Delta\to\Gamma$, the pair $\prod[A,B][\sigma]$ and $\fapp_{A,B}[\sigma]$ can be extended to a dependent product as well, and moreover if $A$ and $B$ are types then their dependent product can be chosen to be a type as well.
  \item \E has \textbf{strictly stable dependent products} if we have an operation assigning to each $A$ and $B$ a dependent product, which is a type if $A$ and $B$ are, and such that all the data (including the operation $\lambda$) commutes strictly with pullback.
  \item \E has \textbf{pseudo-stable dependent products} if we have an operation assigning to each $A$ and $B$ a dependent product which is a type if $A$ and $B$ are, together with for any $\sigma:\Gamma'\to\Gamma$ and cartesian liftings $\sigma_A: A' \to A$ over $\sigma$ and $\sigma_B: B'\to B$ over $\sigma.\sigma_A$, a cartesian arrow $\prod[A',B'] \to \prod[A,B]$ over $\sigma$, that is functorial and commutes with all the structure.
  \end{itemize}
\end{definition}

In fact, all of our examples will have at least pseudo-stable dependent products, because they arise from categorical exponentials.
As remarked in~\cite[3.4.4.9--10]{lw:localuniv}, this simplifies the presentation of further type formers whose rules involve $\prod$-types, so we will henceforth assume it.
The same is true for $\sum$-types and unit types.
...
\begin{definition}
  ...
  \item \todo{Should we define pseudo-stable?}
  \end{itemize}
\end{definition}
