...
Any type family $\jd{x:A |- B(x) \pretype}$ also has a dependent sum $\sm{x:A} B(x)$ satisfying the usual rules, including the $\eta$-rule.
...
It presupposes that $A$ and $B$ are pretypes.

\[ \inferrule{\Gamma \vdash \cof{i}{A}{B}}{\Gamma \vdash i : A \to B} \]
(Perhaps this should be only an admissible rule?)
In particular, when $\Gamma \vdash \cof{i}{A}{B}$ it must be that $\Gamma \vdash A \pretype$ and $\Gamma \vdash B \pretype$.

We will have one axiomatic cofibration, whose codomain is an axiomatic type.
\[ \inferrule{\ }{\Gamma \vdash \ivl \pretype} \qquad 
\inferrule{\ }{\Gamma \vdash 0 : \ivl} \qquad
