...
\begin{proof}
  ...
  \end{equation}
  However, noting that $\sigma$ is an arbitrary object of the slice category $\E/\Gamma$, and $\sigma.\sigma_A$ is its pullback along $\chi(A)$, we see that this says exactly that $\chi(\prod[A,B])$ is a \emph{categorical} dependent product of $\chi(B)$ along $\chi(A)$, i.e.\ the value at $\chi(B)$ of a partially defined right adjoint to the pullback functor $\chi(A)^* : \E/\Gamma \to \E/\Gamma.A$.

  Now it is a standard category-theoretic fact that categorical dependent products are preserved by pullback, up to canonical isomorphism.
