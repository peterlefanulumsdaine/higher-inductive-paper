...
Firstly, it is important that the $\fap$ appearing in the computation rules is the primitive operation introduced in \cref{sec:type-theory}.
...
Secondly, note that the (judgmental!) computation rules for point-constructors are necessary in order for the computation rules for path-constructors to be well-typed.
...
However, when considering \emph{higher} path-constructors---or more generally, path-constructors whose source and target refer to previous path-constructors---if we are restricted to propositional computation rules for lower path-constructors, then there is no escape.

Thirdly, the elimination and computation rules are where the requirement of \emph{naturality} that we briefly mentioned before comes in.
...
Thus, the best we can do is an equality such as
\[ \opp{\fap_{\ct}} \ct \ap{\ap{f}}(\fsurf) \ct \fap_{\ct} \jdeq f_\fsurf. \]
It's hard to make sense of this as a judgmental equality, though\dots
\msnote{Oops, have to go.  To do: pseudo naturality, fibrancy.}
