...
\begin{defn}
  ...
  \end{enumerate}
  An \textbf{excellent model category\footnote{Note that this nonce definition is different from the similarly-named~\cite[Definition A.3.2.16]{lurie:higher-topoi}, though it shares some of the same conditions.}} is a good model category that is in addition combinatorial, i.e.\ it satisfies the following further properties.
  \begin{enumerate}[resume]\ifmpcps\setcounter{enumi}{5}\fi
...
Assumption~\ref{item:m0} tells us that \sM is simplicially enriched, with powers and copowers\footnote{Also known as cotensors and tensors, respectively.} satisfying the pushout-product and pullback-corner axioms, and (by assumption~\ref{item:m3}) preserved by pullbacks.
...
\begin{proof}
  ...
  \[\begin{tikzcd}
    & \sigma^* A+\sigma^*A \arrow[dl,cof] \arrow[rr,cof,acyc] \arrow[dd] \ar[dr,phantom,"R"{name=sr}]
    & & A+A \arrow[dl,cof] \arrow[dd] \ar[to=sr] \\
    \sigma^*A\ten\ivl\arrow[dd] \ar[to=sr, crossing over] & & A\ten\ivl \ar[from=sr] \\
    & \sigma^*B_1+\sigma^*B_2 \arrow[dl] \arrow[rr,cof,near end,"\sim"] \ar[dr,phantom,"S"{name=tr}]
    \ar[from=sr,to=tr,crossing over]
    \arrow[from=ul,to=ur, crossing over,cof,near start,"\sim"']
...
In the case when the dependent identity types and identity applications are defined from the ordinary identity types as \ifmpcps described preceding \cref{defn:dep-id}\else on page~\pageref{idover}\fi, this yields exactly the notion of pushout type used in~\cite[\sec6.8]{hottbook}.
...
\begin{cor}
  For any good model category $\sM$, the split comprehension category $\fibmbang$ has strictly stable weak typal pushouts, relative to the strictly stable identity types obtained by strictifying its canonical weakly stable ones, the dependent identity types obtained from any of~\ref{item:idover1}--\ref{item:idover3} \ifmpcps preceding \cref{defn:dep-id}\else on page~\pageref{idover}\fi, and the identity applications obtained from $\Id$-elimination.\qed
\end{cor}
...
Now let $\iF$ be the higher inductive type generated by the following constructors (with path-constructors numbered~\ref{item:f1}--\ref{item:f5} to match the axioms in \cite{blass:freealg}).
...
\begin{proof}
  ...
  \[ \mathclap{
  \begin{tikzcd}[column sep={2.2cm,between origins},ampersand replacement=\&]
...
\begin{eg} \label{eg:non-rec}
  ...
  \[ \mathclap{
  \begin{tikzcd}[column sep={0.8cm,between origins},ampersand replacement=\&]
    \emptyset \ar[rrr] \&\&[3cm]\& F^{\emptyset\to(B_1+B_2)} \ar[rrr] \&[0.7cm]\&[3cm]\& F \\
    \& \emptyset \ar[ul] \ar[r] \ar[urr,phantom,pos=0.88,"\llcorner"] \& F^{\emptyset \to (B_1+B_2)} \ar[ur] \&
    \& F^{\emptyset\to A} \otimes \mathbf{2} \ar[ul,"{[\inl\circ f_1,\inr\circ f_2]}" description,pos=0.45] \ar[r] \ar[urr,phantom,pos=0.71,"\llcorner"] \& F^{\emptyset\to A} \otimes \ivl \ar[ur]
  \end{tikzcd}
  } \]
