...
\begin{defn}
  ...
  \begin{itemize}
  \item For each $A\in \T(\Gamma)$, a non-empty family of ``good identity types'' $\Id_A \in \T(\Gamma.A.A)$.
    These must be stable under reindexing, in that if $\Id_A$ is a good identity type for $A$ then $\Id_A[\sigma]$ is good for $A[\sigma]$.
  \item For each good identity type, a non-empty family of ``good reflexivity terms'' that are sections of the comprehension $\Gamma.A.\Id_A[\delta_A] \to \Gamma.A$, stable under reindexing (if \r is a good reflexivity term for $A$ then $\r[\sigma]$ is good for $A[\sigma]$).
    % factorizations $A \to P_\Gamma A \fib A\times_\Gamma A$ of the diagonal in which the second factor is a fibration, stable under reindexing in the sense that the pullback of any good factorization along any map $\sigma:\Delta\to\Gamma$ is again good.
  \item For each good identity type $\Id_A$ and good reflexivity term $\r$, and each type $C\in\T(\Gamma.A.A.\Id_A)$ equipped with a section $c$ of $C[\delta_A][\r]\in\T(\Gamma.A)$, a non-empty family of ``good extensions'' of $c$ to a section of $C$ itself, stable under reindexing (the reindexing of a good extension of $c$ is a good extension of $c[\sigma]$).
    Of course every good extension must actually be an extension, i.e.\ its composite with $\delta_A \circ \r$ must be $c$.
    %$A \to P_\Gamma A \fib A\times_\Gamma A$ and each fibration $C\fib A\times_\Gamma A$ equipped with a section $c:A\to C$ over the diagonal, a non-empty family of ``good'' extensions of that section to $P_\Gamma A$.
  \end{itemize}
\end{defn}

Note that a good identity type together with a good reflexivity term consist equivalently of a factorization of $\Gamma.A \to \Gamma.A.A$ through the comprehension $\Gamma.A.A.\Id_A \to \Gamma.A.A$ of some type $\Id_A$.
...
\begin{thm}\label{thm:stable-id}
  ...
  \begin{itemize}
  \item the good factorizations of a fibration $p:\Gamma.A\fib\Gamma$ are those for which $\Gamma.A.A.\Id_A$ is isomorphic to the simplicial power $p^\ivl$ in the slice category over $\Gamma$, with its canonical factorization, and
  \item for any $C$ and $c$, every extension of $c$ to $\Id_A$ is good.
  \end{itemize}
\end{thm}
\begin{proof}
...
By the adjointness between simplicial powers and copowers, a simplicial homotopy $A\ten\ivl \to B$ between $f,g:A\toto B$ is equivalently a lift of $(f,g):A\to B\times B$ to $\Id_B$, i.e.\ a term of $\Id_B[(f,g)]$ in context $A$.
But in order to rephrase \cref{defn:ivl-pushout} relative to a stable class of identity types, we also need to postcompose a homotopy with a function, and we need to talk about ``homotopies over homotopies''.
