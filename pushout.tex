...
\begin{anfxnote}{}
  It would be nice to have each $\sM/\Gamma$ enriched over \sM itself as a cartesian monoidal model category, so that the cofibrations in a cell monad could be cofibrations in \sM.
  However, right now I don't see whether a right proper Cisinski model category is necessarily a cartesian monoidal model category.
\end{anfxnote}
...
This would entail giving a precise \emph{syntactic} definition of what a ``higher inductive type'' is, which is already a significant task.
...
\begin{eg}
  ...
  Similarly, we can construct $S^2$, the torus $T^2$, and so on.
...
\begin{eg}
  ...
  In each case we have a cell monad with two cells, but now it no longer arises from a cell endofunctor.
  \[
  \begin{tikzcd}
...
\begin{eg}
  ...
  The second and fourth cells are both $\emptyset \to F^{T^*S \to T}$, where $T^*S$ denotes the pullback of $S\to I$ along $T\to I$.

  The third cell is $F^{S^*S\to S} \otimes (\mathbf{2}\to \ivl)$ and the fifth cell is $F^{T^*T\to T} \otimes (\mathbf{2}\to \ivl)$.
  TODO: Describe attaching maps.
\end{eg}
