...
TODO: remark on correspondence to syntax, constructors are cells, homotopies become identity types (can do formally in the 1-dimensional case?), give examples


\section{Cell monads with parameters}
\label{sec:cell-monads-param}

Now we generalize the constructions of \cref{sec:cell-monads} to allow parameters, i.e.\ premises for the formation and introduction rules.
Because our type theory has no universes, we have to treat type parameters separately from term parameters.
We simplify things further by assuming that none of the type parameters depend on any of the term parameters, as is frequently the case.
With $\Pi$-types and $\Sigma$-types, this allows us to wrap up all the term parameters into a term belonging to a single type.
For instance, the parameters of a pushout (\cref{sec:pushouts-type-theory}) are equivalent to three types $A,B_1,B_2$ together with a single term of type $(A\to B_1)\times(A\to B_2)$.

However, we do allow type parameters to depend on previous type parameters, as for instance with $\W$-types, whose parameters are a type $A$ and a type $x:A \types B\type$ dependent on $A$.
We encode the ``shape'' of dependency among the type parameters by a \textbf{finite inverse category} \I (equivalently, a skeletal finite category with no nonidentity endomorphisms), whose objects represent types and whose morphisms represent dependencies.
For instance, the type parameters of a pushout are described by the discrete category with three objects, while those of a $\W$-type are described by the arrow category $(\cdot\to\cdot)$.

Recall that for any diagram $\Theta:\I\to\C$ with \I finite and inverse, the \textbf{matching object} at $x\in\I$ is the limit $M_x\Theta = \lim_{y\in x\sslash\I} \Theta(y)$ (if it exists), where $x\sslash \I$ is the full subcategory of the co-slice $x/\I$ that omits the identity $1_x$.
If $\cM$ is a class of morphisms in \C closed under pullbacks, we say that a natural transformation $A\to B$ of \I-diagrams is a \textbf{Reedy \cM-morphism} if for all $x\in \I$ the matching objects $M_x A$ and $M_x B$ exist and the induced map $A_x \to B_x\times_{M_x B} M_x A$ is in \cM.
Assuming \C has a terminal object, we say $A$ is \textbf{Reedy \cM-object} if $A\to 1$ is a Reedy \cM-morphism.
Important choices of \cM include:
\begin{itemize}
\item If \C is a model category (or more generally a fibration category), we can take \cM to be the fibrations.
\item If \C is a comprehension category, we can take \cM to be the \emph{dependent projections}: maps that are isomorphic to the comprehension $\Gamma.A\to \Gamma$ of some type.
\item If \C is a comprehension category, we can also take \cM to be the \emph{display maps}: composites of finite strings of dependent projections.
\item If \C is any category at all, we can take \cM to be the \emph{pullbackable maps}: those of which all pullbacks exist.
\end{itemize}
Thus we have Reedy fibrations, Reedy dependent projections, Reedy display maps, and Reedy pullbackable maps.
The following lemmas are well-known for Reedy fibrations in a fibration category, but we re-prove them to make it clear that they depend on no properties of \cM other than its closure under pullbacks.

% The \textbf{height} of a finite inverse category is the number of objects in the longest string of composable nonidentity arrows.
% In particular, the height of a discrete category is 1 and the height of the empty category is 0.

\begin{lem}
  If \C is a category with a terminal object and $A$ is a Reedy \cM-object in \C, then $\lim A$ exists.
\end{lem}
\begin{proof}
  By induction on the number of objects in \I.
  % If \I has height 0, then it is empty and $\lim A = 1$.
  % Otherwise, induct on the number of objects in \I.
  If \I has no objects, then $\lim A = 1$.
  Otherwise, let $x\in\I$ be an object that is not the target of any nonidentity morphism (such an object exists since \I is finite, inverse, and nonempty).
  Then $\I\setminus\{x\}$ has fewer objects than \I, and restriction to it preserves matching objects, so $A|_{\I\setminus\{x\}}$ is also a Reedy \cM-object and hence has a limit.
  % And $x\sslash \I$ has smaller height than \I, so $\lim A|_{x\sslash \I}$ also exists.
  Moreover, since $A$ is a Reedy \cM-object, $\lim A|_{x\sslash \I} = M_x A$ also exists and $A_x \to M_x A$ is in \cM.
  Now $\lim A$ is the following pullback:
  \[
  \begin{tikzcd}
    \lim A \ar[r] \ar[d] & A_x \ar[d] \\
    A|_{\I\setminus\{x\}} \ar[r] & M_x A
  \end{tikzcd}
  \]
  which exists since \cM is closed under pullbacks.
\end{proof}

In particular, the existence of matching objects is not really an extra hypothesis in the definition of Reedy \cM-objects, since as soon as $A$ is Reedy ``below $x$'' then $M_x A$ is a limit of a Reedy object and hence exists.

\begin{lem}
  If $f:A\to B$ is a Reedy $\cM_f$-morphism, $A$ and $B$ are Reedy \cM-objects), then $\lim f: \lim A \to \lim B$ is a composite of morphisms in \cM (hence in \cM itself, if \cM is closed under composition).
\end{lem}
\begin{proof}
  Cogluing lemma.
\end{proof}

If $(\C,\T)$ is moreover a comprehension category, then $\Theta:\I\to\C$ is \textbf{Reedy fibrant} if for each $x\in\I$, the matching object $M_x\Theta$ exists and there is a specified $\Theta_x \in \T(M_x\Theta)$ whose comprehension is equal to the canonical map $\Theta(x) \to M_x\Theta$.
% It is \textbf{Reedy fibrant} if we only assume that $\Theta_x \in \T(M_x\Theta)$ is a display map (a finite composite of unspecified comprehensions).
The existence of matching objects is not actually an extra condition; as soon as $\Theta$ is Reedy fibrant ``below $x$'', then $M_x\Theta$ automatically exists.
More generally, the limit of any Reedy fibrant diagram always exists.
See~\cite{shulman:invdia} for more details about Reedy fibrant diagrams in comprehension-category-like structures.

Let $\C^\I\f$ denote the category of Reedy fibrant \I-diagrams in slice categories $\C/\Gamma$ (with its comprehension category structure induced by pullback along the domain projection $\C/\Gamma\to\C$), with only cartesian morphisms between them.
Then the projection $\C^\I\f \to\C$ is a fibration of groupoids.

\begin{defn}
  Let $(\C,\T)$ be a comprehension category.
  A \textbf{parameter scheme} over \C consists of a finite inverse category \I and a cartesian functor $\P:\C^\I\f \to \T$ over $\C$.
\end{defn}

The idea is that \P assigns to each collection of type parameters the type whose terms collect all the term parameters.
For instance, for pushouts we have $P(A,B_1,B_2) = (A\to B_1)\times (A\to B_2)$.

\begin{defn}
  An \textbf{instantiation} of a parameter scheme $(\I,\P)$ over $\Gamma\in\C$ consists of a diagram $\Theta \in (\C^\I\f)_\Gamma$ and a section $\theta$ of $\Gamma.\P(\Theta)\to\Gamma$.
  As $\Gamma$ varies, such instantiations form another fibration of groupoids $\cInst(\P) \to \C$.
\end{defn}

\begin{defn}
  A \textbf{fibred category of structures with parameters} over a comprehension category $(\C,\T)$ consists of a parameter scheme $(\I,\P)$, a fibration $\S\to \C$, and a faithful amnestic isofibration $\S \to \cInst(\P)\times_C C^\to$ over \C.
\end{defn}

The objects of the fiber over $(\Theta,\theta,X\to\Gamma)$ will be called \textbf{$\S(\Theta,\theta)$-structures on $X$}; as before they form a partially ordered set.

\begin{defn}
  Let $(\C,\T)$ be a comprehension category and \S a fibred category of structures with parameters over \C.
  For any instantiation $(\Theta,\theta)$ of the parameter scheme of \S over $\Gamma\in \C$, a \textbf{typal initial $\S(\Theta,\theta)$-algebra} consists of a type $H\in\T(\Gamma)$ and an $\S(\Theta,\theta)$-structure on $\Gamma.H\to \Gamma$, such that for any $C\in\T(\Gamma.H)$ together with an $\S(\Theta,\theta)$-structure on the composite $\Gamma.H.C\to\Gamma.H\to\Gamma$ such that $\Gamma.H.C\to\Gamma.H$ is an $\S(\Theta,\theta)$-morphism, there exists a section $\Gamma.H \to \Gamma.H.C$ of $\Gamma.H.C\to\Gamma.H$ that is also an \S-morphism.

  We say $\C$ has \textbf{weakly stable typal initial \S-algebras} if for any $\Gamma$ and $(\Theta,\theta)$ there exists a typal initial $\S(\Theta,\theta)$-algebra whose reindexing along any $\sigma:\Delta\to\Gamma$ is a typal initial $\S(\Theta[\sigma],\theta[\sigma])$-algebra.
  If $\C$ is split, we say it has \textbf{strictly stable typal initial \S-algebras} if we have an operation assigning to each $\Gamma$ and $(\Theta,\theta)$ a typal initial $\S(\Theta,\theta)$-algebra over $\Gamma$ with specified $\S$-algebra sections, in a way that is strictly preserved by the split reindexing functors.
\end{defn}

\begin{defn}\label{defn:replift-param}
  Let $(\C,\T)$ be a comprehension category and \S a fibred category of structures with parameters over \C.
  Given $A\in\T(\Gamma)$ and $B\in\T(\Gamma.A)$ and an instantiation $(\Theta,\theta)$ of the parameter scheme over $\Gamma$, and any $\S(\Theta,\theta)$-structure $\sA$ on $\Gamma.A\to\Gamma$, an \textbf{\S-lift} of $\sA$ to $B$ is an $\S(\Theta,\theta)$-structure on the composite $\Gamma.A.B\to\Gamma.A\to\Gamma$ such that $\Gamma.A.B\to\Gamma.A$ becomes an $\S(\Theta,\theta)$-morphism over $\Gamma$.
  We say that \S has \textbf{representable lifts} if for any $A,B,\Theta,\theta,\sA$, the functor $(\C/\Gamma)\op \to \mathbf{Set}$ defined by
  \begin{equation*}
    (\sigma:\Delta\to\Gamma) \mapsto \{ \S(\Th[\si],\th[\si])\text{-lifts of $\sA[\sigma]$ to $B[\sigma]$}\}
  \end{equation*}
  is representable.
  In other words, there exists a map $\varpi:V_{\sA,B}\to\Gamma$ and an $\S$-lift $\sB$ of $\sA[\varpi]$ to $B[\varpi]$, such that for any $\sigma:\Delta\to\Gamma$ and any $\S$-lift $\sB'$ of $\sA[\sigma]$ to $B[\sigma]$, there is a unique map $\tau:\Delta\to V_{\sA,B}$ such that $\varpi \circ \tau = \sigma$ and $\sB' = \sB[\tau]$.
\end{defn}
...
\begin{lem}
  Let $(\C,\T)$ be a comprehension category and \S a fibred category of structures with parameters over \C.
  If \C has weakly stable typal initial \S-algebras, \C satisfies~\eqref{eq:lf}, and \S has representable lifts, then $\C_!$ has strictly stable typal initial \S-algebras.
\end{lem}
\begin{proof}
  Suppose given $\Gamma\in\C$ and an instantiation $(\Th,\th)$ over $\Gamma$ of the parameter scheme $(\I,\P)$ of \S with respect to $\T_!$.
  Since $\Th$ is a Reedy fibrant $\I$-diagram in $\C/\Gamma$, each map $\Th(x) \to M_x\Th$ is the comprehension of some $\Th_x \in \T_!(M_x\Th)$, which has the form $(V_{\Th_x},E_{\Th_x},\name{\Th_x})$.

  Now the category of Reedy fibrant \I-diagrams in $\C$ is equivalently the ``Reedy fibrant oplax limit'' ${\oplim \I {\C}} \f$, in the sense of~\cite[\sec12]{shulman:invdia}, of the constant functor at $\C$.
  For each $x\in\I$, consider $V_{\Th_x}$ to be a ``universe'' in the copy of $\C$ that is the image of $x$ under this constant functor.
  Of course, since it is only a \emph{local} universe, it need not be closed under any type constructors; but the construction in~\cite[\sec12]{shulman:invdia} of a universe $V$ in ${\oplim \I {\C}} \f$ from one universe in each image category still works as long as the relevant limits and exponentials exist.
  [TODO: Does~\eqref{eq:lf} in a comprehension category suffice for that?  Or do we need the full fibration category structure?]  

  This $V$ is itself an \I-diagram in \C that is Reedy fibrant with respect to the display maps.
  It has the universal property that maps $A\to V$ of \I-diagrams classify ``Reedy small-fibrations'' with respect to the universes $V_{\Th_x}$, i.e.\ transformations $B\to A$ such that $B_x \to M_x B \times_{M_x A} A_x$ is the comprehension of $(V_{\Th_x},E_{\Th_x},\name{V_x})$ for some map $\name{V_x}$.
  Thus, if we let $\Vhat$ be the limit of $V$ (which exists by Reedy fibrancy), then $\Vhat$ is an object of \C with the universal property that maps $\Gamma\to \Vhat$ (which are the same as maps from the constant \I-diagram at $\Gamma$ into $V$) classify \I-diagrams over $\Gamma$ that are Reedy fibrant in $\T_!$ with the local universe at $x\in\I$ being $V_{\Th_x}$.

  In particular, there is a universal such diagram $\Xi$ over $\Vhat$, which is \emph{a fortiori} also Reedy fibrant in $\T$.
  Thus, the parameter scheme \P takes $\Xi$ to a type $\P(\Xi)\in\T(\Vhat)$, whose comprehension has the universal property that maps $\Gamma\to \Vhat.\P(\Xi)$ classify instantiations of \P over $\Gamma$, with a universal instantiation $(\Xi,\xi)$ over $\Vhat.\P(\Xi)$.
  Thus, we take $V_H = \Vhat.\P(\Xi)$ as the local universe for the formation and introduction rules.

  Specifically, let $E_H\in\T(V_H)$ be a weakly stable typal initial $\S(\Xi,\xi)$-algebra, with $\S(\Xi,\xi)$-structure $\sH$.
  Our given instantiation $(\Th,\th)$ is classified by a map $\name{H}:\Gamma\to V_H$, and we take $(V_H,E_H,\name{H})\in\T_!(\Gamma)$ as the specified typal initial $\S(\Th,\th)$-algebra in our intended strictly stable structure for $\T_!$.
...
\begin{defn}
  If $(\C,\T)$ is a comprehension category with pullbacks, a \textbf{fibred monad with parameters} on \C is a parameter scheme $(\I,\P)$ together with a cartesian morphism $\dT:\cInst(\P) \to \cMnd(\C)$ over \C.
\end{defn}

Thus, for any instantiation $(\Th,\th)$ over $\Gm$, we have an indexed monad $\dT(\Th,\th)$ on $\C/\Gm$, whose fiber over $1_\Gm$ is an ordinary monad $\dT(\Th,\th)_\Gm$ on $\C/\Gm$.
We write $\dT\alg_{(\Th,th)}$ for the category of algebras of $\dT(\Th,\th)_\Gm$, each of which has an underlying object of $\C/\Gamma$, i.e.\ of $\C^\to$ over $\Gm$.
These categories vary functorially with respect to cartesian morphisms of $(\Th,\th)$ (in particular, isomorphisms and reindexing along morphisms in \C), so we have a fibration $\dT\alg \to \C$ with a projection $\dT\alg \to \cInst(\P)\times_\C \C^\to$ over \C.
...
\begin{defn}
  If \sM is an excellent model category, then a \textbf{(finite) cell monad with parameters} on \sM is a fibred monad with parameters such that each fibred monad $\dT(\Th,\th)$ is a (finite) cell monad on $\sM/\Gamma$.
\end{defn}
...
\end{thm}
\begin{proof}
  Note that for any $\sigma:\Delta\to\Gamma$ and any instantiation $(\Th,\th)$ over $\Gm$, the monad $\dT(\Th[\si],\th[si])_\De$ (the value of the fibred monad $\dT(\Th[\si],\th[si])$ on $\C/\Delta$ at $1_\Delta$) coincides with the monad $\dT(\Th,\th)_{\si}$ (the value of the fibred monad $\dT(\Th,\th)$ on $\C/\Gamma$ at $\si$).
...
\end{thm}
\begin{proof}
  Inspecting \cref{defn:replift,defn:replift-param}, we see that \dT has representable lifts just when each fibred monad $\dT(\Th,\th)$ on $\C/\Gamma$ does.
