...
Intuitively, a ``parameter scheme'' is a list of hypothized judgments in type theory; but in this section we take a semantic point of view instead.

Recall that in a comprehension category $(\C,\T)$, a \textbf{display map} is a finite composite of maps that are isomorphic to comprehensions of types $\Gamma.A\to\Gamma$.
All pullbacks of display maps exist and are display maps.
We view a display map as a semantic version of a \emph{context extension}.
Let $\D$ be the category whose objects are display maps and whose morphisms are pullback squares; then the codomain projection $\D\to\C$ is a fibration of groupoids.
...
And let $\T_{*}$ be the category whose objects are types equipped with a section of their comprehension $\Gamma.A\to\Gamma$, and whose morphisms are cartesian ones commuting with the sections.
The forgetful functor $\T_{*} \to \T_{\cong}$ is a discrete fibration.

\begin{defn}
  Let $(\C,\T)$ and $(\C,\T')$ be two comprehension structures on the same category
  (think of $\T'=\T$ or $\T'=\T_!$.)
  We define the set of \textbf{parameter schemes over $(\C,\T)$} together with, for every such parameter scheme $\P$, a fibration $\cInst_{\T'}(\P)\to\C$ of \textbf{instantiations of \P with respect to $\T_!$}, by mutual induction as follows.
  \begin{itemize}
  \item There is an \textbf{empty parameter scheme} $\emptyps$, and $\cInst_{\T'}(\emptyps) = \C$ (i.e.\ there is a unique instantiation of $\emptyps$ over every $\Gamma\in\C$).
  \item If \P is a parameter scheme, an \textbf{extension of \P by a type parameter} is a new parameter scheme $\typeps{\P}{\cA}$ determined by a cartesian functor $\cA:\cInst_{\T'}(\P)\to\D$ over \C.
    Its instantiations are determined by the following pullback:
    \[
    \begin{tikzcd}
      \cInst_{\T'}(\typeps{\P}{\cA}) \ar[rr] \ar[d] \pullback{dr} && (\T')_{\cong} \ar[d,"\mathrm{cod}"] \\
      \cInst_{\T'}(\P) \ar[r,"\cA"] \ar[d] & \D \ar[r,"\mathrm{dom}"] & \C\\
      \C
    \end{tikzcd}
    \]
  \item If \P is a parameter scheme, an \textbf{extension of \P by a term parameter} is a new parameter scheme $\termps\P\cA\cB$ determined by a cartesian functor $\cA:\cInst_{\T'}(\P)\to\D$ over \C and a cartesian functor $\cB:\cInst_{\T'}(\P) \to (\T')_{\cong}$ over $\cInst_{\T'}(\P) \xto{\cA} \D \xto{\mathrm{dom}} \C$.
    Its instantiations are determined by the following pullbacks:
    \[
    \begin{tikzcd}
      \cInst_{\T'}(\termps{\P}{\cA}{\cB}) \ar[rr] \ar[d] \pullback{dr} & & (\T')_{*} \ar[d] \\
      \cInst_{\T'}(\P) \ar[dr,"\cA"] \ar[d] \ar[rr,"\cB"] & \phantom{.} & (\T')_{\cong} \ar[d,"\mathrm{cod}"]\\
      \C & \D \ar[r,"\mathrm{dom}"] & \C
    \end{tikzcd}
    \]
  \end{itemize}
  If $\T'=\T$, we omit it from the notation.
\end{defn}

Intuitively, $\cA$ assigns the context extension in which a type or term parameter lives, while $\cB$ assigns the type to which a term parameter belongs (which can depend on the context extension).
For instance, if a type parameter is just $\Gamma\types B\type$, then \cA will assign to every instantiation over $\Gamma$ the terminal display map $1_\Gamma$.
On the other hand, if the parameter scheme \P already contains a type parameter $A$, then the additional type parameter $\Gamma,x:A \types B\type$ will have a functor $\cA$ that assigns to each instantiation over $\Gamma$ the comprehension of the corresponding type $A$.
...
Now we can prove the local universes coherence theorem.
The morphism $\T_!\to\T$ induces (by composition at every level) morphisms $\cInst_{\T'}(\P) \to \cInst_{\T}(\P)$ for any parameter scheme \P over $(\C,\T)$

Note that the notion of parameter scheme, instantiation, and fibred category of structures with parameters depend only on the dependent projections and display maps of a comprehension category, and that the dependent projections and display maps of $\C_!$ are exactly those of $\C$.
(This is why we used $\D_1$ instead of $\T$ in these definitions.)
Thus, any fibred category of structures with parameters over $\C$ is also one over $\C_!$.
...
\end{lem}
\begin{proof}
  Suppose given $\Gamma\in\C$ and an instantiation $\Th$ over $\Gamma$ of the parameter scheme $\P$ of \S.
  We construct a local universe $V_\Th$ for the formation and introduction rules (i.e.\ the type $H\in\T(\Gamma)$ and its $\S(\Th)$-structure), an instantiation $\Xi$ over it, and a map $\name{\Th} : \Gamma\to V_\Th$ such that $\Th = \Xi[\name{\Th}]$, by induction on $\P$.

  In the chase of $\emptyps$, then $V_\Th = 1$, and $\Xi$ and $\name{\Th}$ are unique.

  In the case of $\typeps{\P}{\cA}$, we have inductively $\Th\in\cInst(\P)$ along with $V_\Th$, $\Xi$, and $\name{\Th}$, and also $\cA:\cInst(\P)\to\D$ and a dependent projection $\delta : X \to \mathrm{dom}(\cA(\Th))$.
