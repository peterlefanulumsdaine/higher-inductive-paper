...
\section{Typal initial structures}
\label{sec:init-struct}
...
In this section and the next we consider ``higher inductive types'' without parameters; in later sections we generalize to allow parameters.
...
\section{Typal initial structures with parameters}
\label{sec:init-struct-param}

Now we generalize the constructions of \cref{sec:init-struct,sec:cell-monads} to allow parameters, i.e.\ premises for the formation and introduction rules.
...
\begin{defn}
  Let $(\C,\T)$ be a comprehension category.
  We define the set of \textbf{parameter schemes over $(\C,\T)$} together with, for every such parameter scheme $\P$, a fibration $\cInst(\P)\to\C$ of \textbf{instantiations of \P}, by mutual induction as follows.
  \begin{itemize}
...
\begin{constr}
  ...
  The maps $\rho$ in the two inductive steps are defined by the following diagrams:
    \[
    \begin{tikzcd}
      \cInst(\typeps{\P}{\al}_!) \ar[dr] \ar[rr] \ar[dd] && (\T_!)_{\cong} \ar[dr] \ar[ddr] \\
    ...
    \[
    \begin{tikzcd}
      \cInst(\termps{\P}{\al}{\be}_!) \ar[r] \ar[d] & \cInst(\termps{\P}{\al}{\be}) \ar[rr] \ar[d] \pullback{dr} & & \D_{1,*} \ar[d] \\
...
\begin{defn}
  If \S is a fibred category of structures with parameters over $\C$, with parameter scheme \P, we define $\S_!$ to be the fibred category of structures with parameters $\P'$ over $C_!$ obtained by pullback along $\rho:$
  \[
  \begin{tikzcd}
...
In fact, nothing we said in this section depends on the particular definition of $\cInst(\P)$ from \cref{sec:init-struct-param}; it could just as well be \emph{any} fibration over $\C$.
The internals of $\cInst(\P)$ are only needed to prove the local universes coherence theorem, \cref{thm:lu-str-param}.
